...
if not GROQ_API_KEY or GROQ_API_KEY == "your_groq_api_key_here":
    print("WARNING: GROQ_API_KEY is not set. Bharat AI chat will not work until you provide a valid API key in the .env file.")
else:
    groq_client = Groq(api_key=GROQ_API_KEY)
GROQ_DEEPSEEK_MODEL = "llama-3.1-8b-instant"
...
async def chat_endpoint(request: Request):
    try:
        ...
        if 'groq_client' not in globals():
            return {"response": "I am currently offline. Please add a valid GROQ_API_KEY in the .env file and restart the server to enable Bharat AI."}
